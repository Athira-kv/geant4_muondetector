...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    G4double scintZ = 3*cm;

    // Gap between scintillators (edge to edge)
    G4double scint_gap = 2*m;

    // Calculate center positions
    G4double scint1_z = scint_gap/2 + scintZ/2;   // +1.015m
    G4double scint2_z = -(scint_gap/2 + scintZ/2); // -1.015m
    ...
    // Upper pair (below Scintillator 1)
    // DC1: 5cm below Scintillator 1
    G4double drift1_z = scint1_z - scintZ/2 - 5*cm - driftZ/2;

    // DC2: 1mm below DC1
    G4double drift2_z = drift1_z - driftZ - 1*mm;

    // Lower pair (above Scintillator 2)
    // DC3: 5cm above Scintillator 2
    G4double drift3_z = scint2_z + scintZ/2 + 5*cm + driftZ/2;

    // DC4: 1mm below DC3
    G4double drift4_z = drift3_z - driftZ - 1*mm;
